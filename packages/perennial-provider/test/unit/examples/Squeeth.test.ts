...
describe('Squeeth', () => {
  ...
  describe('#name', async () => {
    it('returns correct name', async () => {
      expect(await squeeth.name()).to.equal('milli-Squeeth')
    })
  })

  describe('#symbol', async () => {
    it('returns correct symbol', async () => {
      expect(await squeeth.symbol()).to.equal('mSQTH')
    })
  })
  ...
  describe('#sync', async () => {
    it('calls sync on oracle', async () => {
      ...
      const result = await squeeth.connect(user).callStatic.sync()
      await squeeth.connect(user).sync()

      expect(result.price).to.equal(utils.parseEther('0.121'))
  ...
  describe('#atVersion', async () => {
    it('modifies oracle per payoff', async () => {
      ...
      expect(oracleVersion.price).to.equal(utils.parseEther('0.121'))
