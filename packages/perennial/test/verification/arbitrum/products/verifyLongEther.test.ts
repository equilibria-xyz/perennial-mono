...
describe('Product - Long Ether - Arbitrum Verification', () => {
  ...
  it('has the correct parameters and configuration', async () => {
    ...
    expect(await longEther['maintenance()']()).to.equal(utils.parseEther('0.05'))
    ...
    expect(utilizationCurve.targetRate).to.equal(utils.parseEther('0.12'))
    expect(utilizationCurve.targetUtilization).to.equal(utils.parseEther('0.8'))
  })

  it('opens positions', async () => {
    await opensPositions(longEther, signer, deployments, 'arbitrum')
  })
})
