...
describe('Reservoir Oracle Market', () => {
  ...
  it('creates a market', async () => {
    ...
    await market.connect(user).update(0, utils.parseEther('1000'))
  ...
  it('opens a make position', async () => {
    ...
    const market = await createMarket(instanceVars, baycUSDCPayoffProvider, reservoirOracle)
    ...
    expect(await market.latestVersion()).to.equal(INITIAL_VERSION.add(1))
    expectPositionEq((await market.versions(INITIAL_VERSION.add(1)))._position, { maker: POSITION, taker: 0 })
    expectPrePositionEq(await market.pre(), {
      _maker: 0,
      _taker: 0,
      _makerFee: 0,
    ...
    expect((await market.accounts(user.address))._position).to.equal(POSITION.mul(-1))
    expect((await market.accounts(user.address))._pre).to.equal(0)
    expect(await market.latestVersions(user.address)).to.equal(INITIAL_VERSION.add(1))
  })
  ...
  it('closes a make position', async () => {
    ...
    await expect(market.connect(user).update(CLOSE_POSITION, 0))
      .to.emit(market, 'Updated')
  ...
  it('opens a take position', async () => {
    ...
    // User State
    expect((await market.accounts(user.address))._position).to.equal(0)
    expect((await market.accounts(userB.address))._pre).to.equal(TAKE_POSITION)
    ...
    expect(await market.latestVersion()).to.equal(INITIAL_VERSION.add(2))
    expectPositionEq((await market.versions(INITIAL_VERSION.add(2)))._position, {
      maker: MAKE_POSITION,
      taker: TAKE_POSITION,
    })
    expectPrePositionEq(await market.pre(), {
    ...
    expect((await market.accounts(user.address))._position).to.equal(TAKE_POSITION)
    expect((await market.accounts(userB.address))._pre).to.equal(0)
    expect(await market.latestVersions(userB.address)).to.equal(INITIAL_VERSION.add(2))
  })
